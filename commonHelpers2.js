import{E as a}from"./assets/main-b91dffa5.js";import"./assets/vendor-c4bdd18a.js";const o=document.querySelector(".favorites-list");document.querySelector(".favorites-div-without-cards");const r="favoriteExerciseIds",n=new a;n.getExercisesByCategory("muscles","abs",1,6).then(e=>d(e.results));function d(e){const t=e.map(s=>`
  ...
  `).join("");o.insertAdjacentHTML("beforeend",t)}function l(e){return e.charAt(0).toUpperCase()+e.slice(1)}const p=document.querySelectorAll(".card-delete");p.forEach(e=>{e.addEventListener("click",()=>{const t=globalThis.id,s=c(),i=s.indexOf(t);i!==-1&&(s.splice(i,1),localStorage.setItem(r,JSON.stringify(s))),v(t)})});function v(e){const t=document.querySelector(`.exercises-card[data-id="${e}"]`);t&&t.remove()}const u=document.querySelectorAll(".card-start"),g=document.querySelector(".exercise-modal-backdrop");u.forEach(e=>{e.addEventListener("click",()=>{g.classList.remove("backdrop-is-hidden")})});const m=document.querySelectorAll(".exercise-favorite-add-btn");m.forEach(e=>{e.addEventListener("click",()=>{const t=globalThis.id;console.log(t);const s=c();s.includes(t)?console.log("this exersice is already in favorites"):(s.push(t),localStorage.setItem(r,JSON.stringify(s)))})});function c(){const e=localStorage.getItem(r);return e?JSON.parse(e):[]}
